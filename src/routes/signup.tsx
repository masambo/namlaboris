...
export const Route = createFileRoute("/signup")({
  head: () => ({
    meta: [
      { title: "Create account | NamLaboris" },
      { name: "description", content: "Create your NamLaboris account and unlock labour-law AI guidance, ADR case tools, and compliance research." },
    ],
  }),
    ...
    <div className="relative flex min-h-screen items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/[0.04] p-8 backdrop-blur-xl shadow-[0_30px_80px_-32px_oklch(0_0_0_/_0.7)]">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input id="fullName" required value={fullName} onChange={(e) => setFullName(e.target.value)} className="mt-1.5" placeholder="Jane Doe" />
            </div>
            <div>
              <Label htmlFor="organization">Organization</Label>
              <Input id="organization" value={organization} onChange={(e) => setOrganization(e.target.value)} className="mt-1.5" placeholder="Acme Ltd" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <Input id="password" type="password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1.5" placeholder="At least 8 characters" />
          </div>
        ...
        <p className="mt-6 text-center text-sm text-foreground/60">
          ...
          <Link to="/login" className="font-semibold text-[var(--brand-gold-soft)] hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
